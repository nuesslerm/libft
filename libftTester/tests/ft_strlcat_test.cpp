...
int	main(void)
{
	...
	src = (char *)"AAAAAAAAA";
	dest[0] = 'B';
	/* 1 */ check(ft_strlcat(dest, src, 0) == strlen(src) && !strcmp(dest,
			"B"));
	showLeaks();
	dest[0] = 'B';
	...
	showLeaks();
	memset(dest, 'B', 4);
	/* 3 */ check(ft_strlcat(dest, src, 3) == 3 + strlen(src) && !strcmp(dest,
			"BBBB"));
	showLeaks();
	/* 4 */ check(ft_strlcat(dest, src, 6) == 13 && !strcmp(dest, "BBBBA"));
	showLeaks();
	memset(dest, 'C', 5);
	/* 5 */ check(ft_strlcat(dest, src, -1) == 14 && !strcmp(dest,
			"CCCCCAAAAAAAAA"));
	showLeaks();
	memset(dest, 'C', 15);
	/* 6 */ check(ft_strlcat(dest, src, 17) == 24 && !strcmp(dest,
			"CCCCCCCCCCCCCCCA"));
	showLeaks();
	memset(dest, 0, 30);
	...
	memset(dest, 0, 30);
	memset(dest, '1', 10);
	/* 8 */ check(ft_strlcat(dest, src, 5) == strlen(src) + 5 && !strcmp(dest,
			"1111111111"));
	showLeaks();
	memset(dest, 0, 30);
	memset(dest, '1', 10);
	/* 9 */ check(ft_strlcat(dest, src, 5) == strlen(src) + 5 && !strcmp(dest,
			"1111111111"));
	showLeaks();
	memset(dest, 0, 30);
	memset(dest, '1', 10);
	/* 10 */ check(ft_strlcat(dest, "", 15) == 10 && !strcmp(dest,
			"1111111111"));
	showLeaks();
	memset(dest, 0, 30);
